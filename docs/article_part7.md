...
## Стадарты GNU
Бьюсь о заклад, что вы крайне редко видели код на GNU стандартах. В качестве примера я хочу показать пример рабочей программы с устаревшими и специфичными вещами.
...
Этот код использует такие вещи как триграфы  `??=` вместо символа `#`, `??<` для открывающей фигурной скобки и `??>` для закрывающей. Тригрфы и диграфы (`<%` и `%>` также заменяют фигурные скобки, а `<:` и `:>` служат заменой квадратных скобок) были созданы для замены возможных несуществующих на клавиатуре символов скобок в стародавние времена.
...
Наиболее экзотическая часть кода — это использование указателей на метки. Конструкция `&&label` берёт адрес метки в памяти, сохраняя его в указатель `void *ptr`. Затем с помощью оператора косвенного перехода `goto *ptr;` выполняется прыжок по этому адресу. Это нестандартное расширение GCC. После перехода на метку `label:` функция `main` возвращает ранее вычисленную сумму.
