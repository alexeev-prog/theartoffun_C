...
# Код грея
Код Грея, который носит также название зеркального, это двочиный код, в котором две соседние кодовые комбинации различаются только цифрой в одном двочином разряде. Иными словами, расстояние Хэмминга между соседними кодовыми комбинациями равно 1.
...
Преобразование в код Грея очень простое - число n сдвигается на один бит и выполняется XOR между исходным числом и сдвинутым. Для числа 5 (101 в двочиной) сдвиг дает 2 (010), XOR дает 111 (7), что является корректным кодом Грея для 5.

Обратное же сложнее, но использует ту же идею. Алгоритм последовательно применяет XOR с разными сдвигами для восстановления числа. Начинается с наибольшого сдвига (16 бит для 32 битного числа) и постепенно уменьшает сдвиг до 1 бита.
...
# Counting Sort
Алгоритмы сортировки - неисчерпаемая тема. Давайте разберем специфичный метод сортировки массива - подсчетом для малых диапазанов.
...
# ГСПЧ MSWS
Раз уж пошла тема про ГСПЧ, грех будет не рассказать об алгоритме Middle-Square weyl Sequence, предложенного легендарным Джоном фон Нейманом в 1940-х годах.
...
В итоге лидерами среди ГПСЧ по скорости оказываются pcg32, romu_duo и xorshift64. В средней группе msws32, xorshiro256pp, wyrand. А самым медленным стал lehmer64.
